...
//   The describe function is for grouping related specs. The string parameter is for naming the collection
// of specs, and will be concatenated with specs to make a spec's full name. This aids in finding specs in a large suite.
...
// ¶
// Setup and Teardown
// To help a test suite DRY up any duplicated setup and teardown code, Jasmine provides the global beforeEach,
// afterEach, beforeAll, and afterAll functions.
//
// ¶
...
//
//   Here is the same set of specs written a little differently. The variable under test is defined at the top-level scope
// -- the describe block -- and initialization code is moved into a beforeEach function. The afterEach function
// resets the variable before continuing.
...
// ¶
// The beforeAll function is called only once before all the specs in describe are run, and the afterAll
// function is called after all specs finish. These functions can be used to speed up test suites with expensive setup and teardown.
//
//   However, be careful using beforeAll and afterAll! Since they are not reset between specs, it is easy
// to accidentally leak state between your specs so that they erroneously pass or fail.
...
// Calls to describe can be nested, with specs defined at any level. This allows a suite to be composed
// as a tree of functions. Before a spec is executed, Jasmine walks down the tree executing each beforeEach
// function in order. After the spec is executed, Jasmine walks through the afterEach functions similarly.
...
// ¶
// Disabling Suites
// Suites can be disabled with the xdescribe function. These suites and any specs inside them
// are skipped when run and thus their results will show as pending.
...
// ¶
// And if you call the function pending anywhere in the spec body, no matter the expectations,
// the spec will be marked pending. A string passed to pending will be treated as a reason and displayed when the suite finishes.
...
// A spy only exists in the describe or it block in which it is defined, and will be removed after each spec. There are
// special matchers for interacting with spies. This syntax has changed for Jasmine 2.0.
...
// By chaining the spy with and.callThrough, the spy will still track all calls to it but in addition
...
// By chaining the spy with and.returnValues, all calls to the function will return specific values in
// order until it reaches the end of the return values list, at which point it will return undefined for
...
// ¶
// Spies: createSpy
// When there is not a function to spy on, jasmine.createSpy can create a "bare" spy. This spy acts as any other spy
// - tracking calls, arguments, etc. But there is no implementation behind it. Spies are JavaScript objects and can be used as such.
...
// Jasmine also has support for running specs that require testing asynchronous operations. The functions
// that you pass to beforeAll, afterAll, beforeEach, afterEach, and it can be asynchronous. There are three different
// ways to indicate that a function is asynchronous: by taking an optional callback parameter, by returning a promise,
...
//   ¶
// Calls to beforeAll, afterAll, beforeEach, afterEach, and it can take an optional single argument that should
...
// ¶
// Functions passed to beforeAll, afterAll, beforeEach, afterEach, and it can return a promise that should be
// resolved when the async work is complete. If the promise is rejected, all specs in the enclosing describe will fail.
...
// not complete until the promise that it returns is settled. If the promise is rejected, the spec will fail.
